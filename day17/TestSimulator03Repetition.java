...
// Not sure how useful this is. On 4 Jan 24 at 07:13 it finished a run which took 36.12 hours. There were three failures. Notes in 20240104.txt.
// Going to take it out of TestRunner for the moment.

public class TestSimulator03Repetition {

	public static void main(String[] args) throws Exception {
		TestSimulator03Repetition obj = new TestSimulator03Repetition();
	}
	...
	//
	//		p.endTester.maxRocks = numRocks * numJets;
	//		p.debug = false;
	...
	//		int numJets =  p.data.get(0).split("").length;
	//
	//		p.endTester.maxRocks = numRocks * numJets * 2;
	//		p.debug = false;
	//		int out = p.runSimulation();
	//		p.chamber.logger.debug = true;
	...
	Chamber getChamberLCM(int numLCMs) throws Exception{
		...
		p.endTester.maxRocks = numRocks * numJets * numLCMs;
		// p.debug = true;
		int out = p.runSimulation();
		return p.chamber;
	}
	...
	//		boolean same = true;
	//		while (same) {
	//			Simulator p = new Simulator("data.txt");
	//			p.endTester.maxRocks = (int) numRocks;
	//			int out = p.runSimulation();
	//			p.chamber.logger.debug = true;
	//			p.chamber.show();
	//
